...
final class ConnectMySQL extends ConnectDB
{
	...
	protected function connect()
	{
		...
		if (!$this->_link)
			throw new \Exception("Connection refus�e au serveur de base de donn�es! (".mysql_error().")");
		else{
			$this->_success = @mysql_select_db($this->dbname,$this->_link); 
			if (!$this->_success)
				throw new \Exception("Connection refus�e � la base de donn�es! (".mysql_error().")");
		}
	}
	...
	public function query($sql)
	{
		if(is_string($sql)){
			if($result = mysql_query($sql,$this->_link))
				...
				throw new \Exception("Requ�te impossible!");
				return false; 
			}
		}
		else{
			throw new \Exception("La requ�te n'est pas valide!");
			return false; 
		}
	}
	...
	/* Fonctions suppl�menatires li�es � une table */
	...
	public function nbFields($tableName)
	{
		...
		if($result = $this->query($sql))
			if($nb = mysql_num_fields($result))
				...
				throw new Exception("Aucun champ trouv�!");
				return false; 
			}
		else{
			return false; 	
