...
if(@$_REQUEST["action"] == 'add'){
	// Si l'article est d�j� dans le panier, on en ajoute 1, sinon on en met un premier
	$nombre = 0;
	if(isset($_SESSION["panier"][$_REQUEST["id"]]))
		$nombre = $_SESSION["panier"][$_REQUEST["id"]]["nombre"];
		
	// On cherche le titre et le prix actuel
	$prix = 0;
	$sql = "SELECT * FROM articles WHERE id = ".$_REQUEST["id"];

	//$result = $pdo->query($sql);
    $result = App::db()->fetch($sql);
    foreach($result AS $val){
		$titre = $val["title"];
		$prix = $val["price"];
	}
	
	// On ajoute l'article � la session panier
	$_SESSION["panier"][$_REQUEST["id"]] = array('id'=>$_REQUEST["id"], 'titre'=>$titre, 'prix'=>$prix, 'nombre'=>$nombre+1);
